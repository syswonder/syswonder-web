# Syswonder Open Source Community

The **Syswonder Ubiquitous Operating System Open Source Community** (the **Syswonder Community**) is a **non-profit, vendor-neutral open source technical community** dedicated to the research and development of key technologies for next-generation ubiquitous operating systems.

The Community focuses on future ubiquitous intelligent computing scenarios, including **embodied artificial intelligence**, **intelligent manufacturing**, and other **in-situ computing environments**. Its goal is to collaboratively build advanced open source projects with international impact, and to foster **open, cooperative research and innovation** among researchers and engineering professionals from diverse organizations through the joint development of open source projects.

**Focus Areas**

The Community develops open source projects targeting **in-situ computing scenarios** that require real-time perception and situational awareness in near-field environments, including but not limited to:

- **hvisor**: a lightweight hypervisor for heterogeneous edge computing platforms, implemented in Rust and formally verified  
  <https://github.com/syswonder/hvisor>

- **ruxos**: an Unikernel operating system implemented in Rust  
  <https://github.com/syswonder/ruxos>

- **robonix**: an operating system for embodied intelligence  
  <https://github.com/syswonder/robonix>

**Community Governance**

The Community adopts an **open, transparent, and consensus-driven governance model**, consisting of the following bodies and roles:

- **Technical Committee (TC)**: Responsible for defining technical directions, managing projects, and making technical decisions.
- **Open Source Community Committee**: Responsible for ecosystem development, application outreach, and community operations.
- **Community Administrators**: Responsible for day-to-day operations and infrastructure maintenance.


## Technical Committee

The Syswonder Technical Committee defines and maintains the
community's technical vision, guides and builds core community projects.

<div class="card-row">

  <div class="card-column">
    <div class="card">
      <img class="card-img" src="https://yuzhang-nwpu.github.io/yuzhang/photos/zhangyu.png" alt="zy" style="height:200px">
      <div class="container">
        <h4>Yu Zhang, Chair</h4>
        <p>Northwestern Polytechnical <br> University</p>
        <a href="https://teacher.nwpu.edu.cn/yuzhang.html" class="btn btn-primary stretched-link">More</a>
      </div>
    </div>
  </div>

  <div class="card-column">
    <div class="card">
      <img class="card-img" src="https://caodg.github.io/caodg/img/caodg.png" alt="caodg" style="height:200px;">
      <div class="container">
        <h4>Donggang Cao</h4>
        <p>Peking <br> University</p>
        <a href="https://cs.pku.edu.cn/info/1210/2915.htm" class="btn btn-primary stretched-link">More</a>
      </div>
    </div>
  </div>

  <div class="card-column">
    <div class="card">
      <img class="card-img" src="https://www.cs.tsinghua.edu.cn/__local/3/EA/2B/A0177095645B03C810A03FD0EA8_F10993A0_1075E.jpg" alt="cy" style="height:200px">
      <div class="container">
        <h4>Yu Chen</h4>
        <p>Tsinghua <br> University</p>
        <a href="https://www.cs.tsinghua.edu.cn/info/1112/3500.htm" class="btn btn-primary stretched-link">More</a>
      </div>
    </div>
  </div>

</div>

---

<div class="card-row">

  <div class="card-column">
    <div class="card">
      <img class="card-img" src="https://www.cs.tsinghua.edu.cn/__local/2/49/01/382767512A6D170C914F061BA08_814D0E83_1666C.jpg" alt="ck" style="height:200px">
      <div class="container">
        <h4>Kang Chen</h4>
        <p>Tsinghua <br> University</p>
        <a href="http://madsys.cs.tsinghua.edu.cn/~kangchen/" class="btn btn-primary stretched-link">More</a>
      </div>
    </div>
  </div>

  <div class="card-column">
    <div class="card">
      <img class="card-img" src="https://cs.pku.edu.cn/__local/5/2D/6D/9E0EE1D872F8EF7F776D3E0D564_1E54E99E_5355.jpg" alt="gy" style="height:200px">
      <div class="container">
        <h4>Yao Guo</h4>
        <p>Peking <br> University</p>
        <a href="https://cs.pku.edu.cn/info/1061/1259.htm" class="btn btn-primary stretched-link">More</a>
      </div>
    </div>
  </div>

  <div class="card-column">
    <div class="card">
      <img class="card-img" src="https://ict.cas.cn/sourcedb/cn/jssrck/201212/P020221108527707635438.jpg" alt="ld" style="height:200px">
      <div class="container">
        <h4>Dong Li</h4>
        <p>ICT, Chinese Academy <br> of Sciences</p>
        <a href="http://www.ict.cas.cn/sourcedb/cn/jssrck/201212/t20121207_3701870.html" class="btn btn-primary stretched-link">More</a>
      </div>
    </div>
  </div>

</div>

---

<div class="card-row">


  <div class="card-column">
    <div class="card">
      <img class="card-img" src="https://www.cs.sjtu.edu.cn/upload/image/jiaoshiml/薛栋梁.jpg" alt="xdl" style="height:200px">
      <div class="container">
        <h4>Dongliang Xue</h4>
        <p>Shanghai Jiao Tong <br> University</p>
        <a href="https://www.cs.sjtu.edu.cn/jiaoshiml/xuedongliang.html" class="btn btn-primary stretched-link">More</a>
      </div>
    </div>
  </div>

  <div class="card-column">
    <div class="card">
      <img class="card-img" src="https://person.zju.edu.cn/person//attachments/2020-09/0914070934-234338679.jpg" alt="zyw" style="height:200px">
      <div class="container">
        <h4>Yongwang Zhao</h4>
        <p>Zhejiang <br> University</p>
        <a href="https://person.zju.edu.cn/zhaoyw/" class="btn btn-primary stretched-link">More</a>
      </div>
    </div>
  </div>


</div>

## Open Source Community Committee

The Syswonder Open Source  Community Committee builds and maintains the
community ecosystem and develops community partners.

<div class="card-row">
  <div class="card-column">
    <div class="card">
      <img class="card-img" src="https://ict.cas.cn/sourcedb/cn/jssrck/201212/P020221108527707635438.jpg" alt="ld" style="height:200px">
      <div class="container">
        <h4>Dong Li, Chair</h4>
        <p>ICT, Chinese Academy of Sciences </p>
        <a href="http://www.ict.cas.cn/sourcedb/cn/jssrck/201212/t20121207_3701870.html" class="btn btn-primary stretched-link">More</a>
      </div>
    </div>
  </div>

</div>


